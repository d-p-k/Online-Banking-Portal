...
public interface LockerRepository extends JpaRepository<Locker, Integer> {
	...
	public Locker findLockerByLockerId(int lockerid);
	...
	public List<Locker> pendingLockers();
	
	@Query("SELECT l FROM Locker l WHERE l.lockerstatus = 'ACTIVE'")
	public List<Locker> activeLockers();
	
	@Query("SELECT l FROM Locker l WHERE l.lockerstatus = 'CLOSED'")
	...
	@Modifying
	@Transactional 							
    @Query("UPDATE Locker l SET l.lockerstatus = 'REQUESTED FOR CLOSING' WHERE l.account.accid = :accid AND l.lockertype = :lockertype")
    public void closeLockerRequest(int accid,String lockertype);
	
	@Query("SELECT l FROM Locker l WHERE l.lockerstatus = 'REQUESTED FOR CLOSING'")
	...
	@Query("SELECT l.account.accid from Locker l WHERE l.lockerid = :lockerid")
	public int getAccIdByLockerId(int lockerid);
}
