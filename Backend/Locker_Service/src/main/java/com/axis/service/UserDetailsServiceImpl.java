...
public class UserDetailsServiceImpl implements UserDetailsService{
	...
	public void applyForLocker(Users user, String lockertype, String lockerlocation, String lockersize) {
		...
		Locker locker = new Locker(lockertype, lockerlocation, lockersize, lockeramount, "PENDING", findAccountByUserId(user.getId()));
		lr.save(locker);
	...
	public void lockerPayment(String lockertype, double lockeramount, Users user) {
		ar.lockerPayment(lockeramount, findAccId(user.getId()));
		Transaction t=new Transaction(generateTransactionId(), LocalDateTime.now(), lockeramount, lockertype + " Payment Withdrawal", "DEBIT", ar.findAccountByUserId(user.getId()));
		ctr.save(t);
	}
	...
	public List<Locker> pendingLockers() {
		return lr.pendingLockers();
	}
	
	public List<Locker> activeLockers() {
		return lr.activeLockers();
	}
	
	public List<Locker> closedLockers() {
		return lr.closedLockers();
	}
	...
	public Locker findLockerByLockerId(int lockerid) {
		return lr.findLockerByLockerId(lockerid);
	}
}
